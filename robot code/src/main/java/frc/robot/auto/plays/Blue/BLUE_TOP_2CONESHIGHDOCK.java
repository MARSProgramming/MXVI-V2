...
public class BLUE_TOP_2CONESHIGHDOCK extends SequentialCommandGroup{
    public BLUE_TOP_2CONESHIGHDOCK(DrivetrainSubsystem drivetrain, Manipulator mManipulator){
        ...
        PathPlannerTrajectory Dock = AutoChooser.openTrajectoryFileForBlue("BLUE_TOP_P1-G", new PathConstraints(2, 1.5));
        addCommands(
            ...
            new DriveAtPath(drivetrain, P1, false, false, 3.5).deadlineWith(
                mManipulator.goToIntake().alongWith(mManipulator.getGrasper().setPercentOutputCommand(1))
            ),
            new DriveAtPath(drivetrain, ScoreP1, false, false, 3.5).deadlineWith(
                mManipulator.goToZero().until(() -> drivetrain.getPose().getX() < 3.6).andThen(mManipulator.goToCubeShootHigh())
            ),
