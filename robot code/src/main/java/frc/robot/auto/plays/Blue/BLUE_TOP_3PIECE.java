...
public class BLUE_TOP_3PIECE extends SequentialCommandGroup{
    public BLUE_TOP_3PIECE(DrivetrainSubsystem drivetrain, Manipulator mManipulator){
        ...
        PathPlannerTrajectory P1 = AutoChooser.openTrajectoryFileForBlue("BLUE_TOP_G-P1", new PathConstraints(3, 2.5));
        ...
        PathPlannerTrajectory ScoreP2 = AutoChooser.openTrajectoryFileForBlue("BLUE_TOP_P2-G", new PathConstraints(3, 2.5));
        addCommands(
            ...
            new DriveAtPath(drivetrain, ScoreP1, false, false, 3).alongWith(
                mManipulator.goToZero().until(() -> drivetrain.getPose().getX() < 3.2).deadlineWith(mManipulator.getGrasper().runTestCurrent()).andThen(mManipulator.goToScoreHigh().withTimeout(2.2))
            ),
