...
public class BtP1P2_NoDock extends SequentialCommandGroup{
    public BtP1P2_NoDock(DrivetrainSubsystem drivetrain, Manipulator mManipulator){
        ...
        PathPlannerTrajectory P1toMarker = AutoChooser.openTrajectoryFile("BLUE_TopMarker_P1-M", new PathConstraints(2, 0.75));
        PathPlannerTrajectory MarkertoP2 = AutoChooser.openTrajectoryFile("BLUE_TopMarker_M-P2", new PathConstraints(2, 0.75));
        PathPlannerTrajectory P2toMarker = AutoChooser.openTrajectoryFile("BLUE_TopMarker_P2-M", new PathConstraints(2, 0.75));
        addCommands(
            ...
            new DriveAtPath(drivetrain, MarkertoP1, 0, 4.4).deadlineWith(
                mManipulator.goToCubeIntake(),
                mManipulator.getGrasper().runTestMode()
            ),
            new ParallelCommandGroup(new DriveAtPath(drivetrain, P1toMarker, 0, 4.5).deadlineWith(
                mManipulator.getGrasper().runTestCurrent()
            ),
            mManipulator.goToZero().withTimeout(2.5).andThen(mManipulator.goToCubeShootHigh().withTimeout(2.0))
            ),
